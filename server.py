...
def customer_nocontact():
 cursor = g.conn.execute("SELECT oi.*, o.order_id, o.order_date, u.cust_id, u.firstname, u.lastname, u.email_address, u.phone FROM orders o JOIN order_items oi ON o.order_id = oi.order_id JOIN users u ON u.cust_id = o.cust_id WHERE u.email_address IS NULL OR u.phone IS NULL")
 g.conn.begin().commit()
...
def bestsell_string():
 cursor = g.conn.execute("SELECT s.string_code, sum(oi.quantity) FROM order_items oi JOIN string s ON oi.item_code = s.string_code GROUP BY s.string_code ORDER BY SUM(oi.quantity) DESC LIMIT 3")
 g.conn.begin().commit()
 record = cursor.fetchall()
 context = dict(data = record)
 print(record)
...
def athlete_search():
	...
	cursor = g.conn.execute("SELECT * FROM athletes a JOIN use_equipment ue ON a.athlete_id = ue.athlete_id WHERE name=%s", (athlete_name))
	g.conn.begin().commit()
...
def racquet_search():
	...
	cursor = g.conn.execute("SELECT * FROM racquet WHERE racquet_code=%s", (racquet_code))
	g.conn.begin().commit()
	record = cursor.fetchone()
	context = dict(data = record)
	print(record)
...
def racquet_budget():
	...
	cursor = g.conn.execute("SELECT name, unit_price FROM racquet WHERE unit_price < %s", (racquet_budget))
	g.conn.begin().commit()
	record = cursor.fetchall()
	context = dict(data = record)
	print(record)
...
def string_budget():
	...
	cursor = g.conn.execute("SELECT string_code, unit_price FROM string WHERE unit_price < %s", (string_budget))
	g.conn.begin().commit()
	record = cursor.fetchall()
	context = dict(data = record)
	print(record)
...
def Delete_racquet():
	racquet_code = request.form['racquet_code']
	cursor = g.conn.execute("DELETE FROM racquet WHERE racquet_code= %s", (racquet_code))
	g.conn.begin().commit()
	select_query = "SELECT * from racquet"
	cursor = g.conn.execute(text(select_query))
	res = []
...
def Delete_string():
	...
	cursor = g.conn.execute("DELETE FROM string WHERE string_code= %s", (string_code))
	g.conn.begin().commit()
	select_query = "SELECT * from string"
	cursor = g.conn.execute(text(select_query))
	res = []
...
def Delete_order():
	...
	cursor = g.conn.execute("DELETE FROM orders WHERE Order_id= %s", (Order_id))
	g.conn.begin().commit()
	select_query = "SELECT * from orders"
	cursor = g.conn.execute(text(select_query))
	res = []
